using Microsoft.AspNetCore.Mvc;
using REAL_ESTATE_CLEAN.Core.Application.Interfaces.Services;

namespace REAL_ESTATE_CLEAN.Presentation.Api.Controllers;

[ApiController]
[Route("api/ai/forecast-analytics")]
public class PropertyAiCustomerSalesLeadForecastAnalyticsController : ControllerBase
{
    private readonly IPropertyAiCustomerSalesLeadForecastAnalyticsService _service;


    public PropertyAiCustomerSalesLeadForecastAnalyticsController(
        IPropertyAiCustomerSalesLeadForecastAnalyticsService service)
    {
        _service = service;
    }


    [HttpGet("{userId}")]
    public async Task<IActionResult> GetAnalytics(int userId)
    {
        return Ok(await _service.GetAnalyticsAsync(userId));
    }


    [HttpGet("{userId}/history")]
    public async Task<IActionResult> GetHistory(int userId)
    {
        return Ok(await _service.GetHistoryAsync(userId));
    }
}
